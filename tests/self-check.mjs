...
const mazeSource = await fs.readFile(path.join(root, 'vendor/dsh-trace-compare/maze-upload.html'), 'utf8')
assert.match(rendererSource, /Trace 对比/)
assert.doesNotMatch(rendererSource, /new MutationObserver/)
assert.doesNotMatch(rendererSource, /Trajectory⌄/)
assert.match(rendererSource, /aria-haspopup="menu"/)
assert.match(rendererSource, /const MENU_ORIGIN = \{ left: 252, top: 52 \}/)
assert.match(rendererSource, /menuLayout = 'fixed'/)
assert.doesNotMatch(rendererSource, /getBoundingClientRect\(\)\.left/)
assert.match(rendererSource, /menuHost\.hidden = !valid/)
assert.match(rendererSource, /stroke="currentColor"/)
assert.match(rendererSource, /width:28px;height:28px/)
assert.match(rendererSource, /position:fixed;z-index:1;visibility:hidden/)
assert.match(rendererSource, /font-weight:400!important/)
assert.match(rendererSource, /box-shadow:none;visibility:hidden/)
assert.match(rendererSource, /width:calc\(100vw - 300px\)/)
assert.match(rendererSource, /openDrawer\('trajectory'\)/)
assert.match(rendererSource, /openDrawer\('compare'\)/)
assert.match(rendererSource, /openDrawer\('v2'\)/)
...
assert.match(hostSource, /await fs\.readFile\(path\.join\(ROOT, 'src\/v2-runtime\.mjs'\), 'utf8'\)/)
assert.match(rendererSource, /#codex-trajectory-maze/)
assert.match(rendererSource, /#codex-trace-compare/)
assert.match(rendererSource, /const frames = new Map\(\)/)
assert.match(rendererSource, /const existing = frames\.get\(mode\)/)
assert.match(rendererSource, /iframe\.hidden = true/)
assert.match(rendererSource, /themeTimer = setInterval\(\(\) => \{ if \(frameReady\) sendAppearance\(\)/)
assert.doesNotMatch(rendererSource, /frameMode !== mode\) removeFrame/)
assert.doesNotMatch(rendererSource, /item\.className = standaloneClass/)
assert.match(mazeSource, /function whyText\(why, why2\)/)
assert.match(mazeSource, /function setLang\(lang\)/)
assert.match(mazeSource, /function turnStartTimes\(lane\)/)
assert.match(mazeSource, /id="metricsOverview"/)
assert.match(mazeSource, /function renderMetrics\(\)/)
assert.match(mazeSource, /结果未评分/)
assert.match(mazeSource, /实际工具调用/)
assert.match(mazeSource, /工具类型/)
assert.match(mazeSource, /<details class="mo-tools"/)
assert.match(mazeSource, /rowspan="\$\{types\.length\}"/)
assert.doesNotMatch(mazeSource, /concreteTools\[`\$\{name\}\.\$\{type\}`\]/)
assert.match(mazeSource, /current\.metrics = lane\.metrics/)
assert.doesNotMatch(mazeSource, /DATA = e\.data\.data/)
// Vendor-upgrade regression guards: v0.5.1 removed both behaviors.
// Keep timeline labels compact and hover cards fully inside the maze viewport.
assert.match(mazeSource, /function showTipFor\(d, e\)/)
assert.match(mazeSource, /function placeTip\(tip, e\)/)
assert.match(mazeSource, /showTipFor\(hoverDet, e\)/)
assert.match(mazeSource, /document\.getElementById\('svgwrap'\)\.getBoundingClientRect\(\)/)
assert.match(mazeSource, /tip\.style\.maxWidth = Math\.max/)
assert.match(mazeSource, /tip\.style\.maxHeight = Math\.max/)
assert.doesNotMatch(mazeSource, /px \+ 640/)
assert.match(mazeSource, /function clipLabel\(value, max = 22\)/)
assert.match(mazeSource, /sl\.textContent = clipLabel\(nodeLabel\(d\)\)/)
assert.match(mazeSource, /const short = clipLabel\(label\)/)
assert.match(mazeSource, /const full = clipLabel\(label \+ \(toolTxt/)
