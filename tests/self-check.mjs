...
const cdpSource = await fs.readFile(path.join(root, 'src/cdp.mjs'), 'utf8')
...
assert.doesNotMatch(rendererSource, /if \(!taskId\(\)\) return/)
assert.match(cdpSource, /!target\.url\.includes\('avatar-overlay'\)/)
